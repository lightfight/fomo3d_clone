...
1. 管理员：构造方法中添加的管理员，对管理员的增加删除
2. 签名次数
3. 管理员有3个属性：名称、是否是开发dev，是否是管理员admin
4. 提供给外部访问的简单接口

*/
contract TeamAnonymous { // 团队匿名

    address private undefined = 0xBd01103c36f400344b427Cb51934B765007e16f6; //接收以太坊的地址
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    public
    {
        ...
        address creator  = 0x3C21550C76B9C0Eb32ceA5f7ea71d54f366961a1; //创建者地址
        address admin    = 0x3123AD3e691bC320aaCC8ab91A0E32A7eE4C4b9a; //管理员地址
    ...
    payable
    {
        //如果有人向这个地址转了eth ， 通过这个方法可以把所有的eth转给 undefined 定义的地址
    ...
    * 更改签名的数量 ， 这个数量值   1 <= _howMany <= admins 的数量
