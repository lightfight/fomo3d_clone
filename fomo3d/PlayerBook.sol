...
1. 包括游戏、玩家、玩家的上家、玩家的名字注册、一个玩家可以有多个名字、玩家可以把自己的名字添加到某个游戏或全部游戏中
2. 注册名字是需要收费的，费用直接转到team账号上，这个费用是可以有接口修改的
3. 注册名字时的上家可以填写address、name、playerId三种方式，最终核心处理类都是registerNameCore，注册名称会广播事件
4. 玩家Player属性有玩家地址、当前名字、上家、拥有名字数量，参看：struct Player

*/
contract PlayerBook {
    using NameFilter for string; // 只要是string类型的，NameFilter的内部方法就可以直接跟在该string变量后面，比如：_nameString.nameFilter();
    using SafeMath for uint256; // 同上

    // TO DO: 团队管理的智能合约
    TeamAnonymousInterface constant private TeamAnonymous = TeamAnonymousInterface(0xcfdc23c0e94cbb322b5cbd568839be377c2843a8);

    MSFun.Data private msData;
    function multiSigDev(bytes32 _whatFunction) private returns (bool) {
        return(MSFun.multiSig(msData, TeamAnonymous.requiredDevSignatures(), _whatFunction));}
    ...
    uint256 public registrationFee_ = 10 finney;            // price to register a name （注册名字的价格）
    ...
    //==============================================================================
    //     _ _  _  _|. |`. _  _ _  .
    //    | | |(_)(_||~|~|(/_| _\  .  (these are safety checks) 安全检查
    //==============================================================================
    /**
    ...
    //    (/_\/(/_| | | _\  .
    //==============================================================================
    // fired whenever a player registers a name  有新用户注册进来时发送的事件
    event onNewName
    (
    ...
    //==============================================================================
    //     _  _ _|__|_ _  _ _  .
    //    (_|(/_ |  | (/_| _\  . (for UI & viewing things on etherscan) 检查名字的有效性
    //=====_|=======================================================================
    ...
    public
    {
        ...
        if (_totalNames > 1)
            for (uint256 ii = 1; ii <= _totalNames; ii++)
                games_[_gameID].receivePlayerNameList(_pID, plyrNameList_[_pID][ii]); // 一个玩家可以有多个名字
    }
    ...
    public
    {
        ...
        // make sure they own the name // 如果玩家拥有这个名字才设置这个名字
        require(plyrNames_[_pID][_name] == true, "umm... thats not a name you own");

        // update their current name // 一个玩家可以拥有多个名字，而且可以选择某个名字作为当前的名字
    ...
    private
    {
        ...
        // registration fee goes directly to community rewards 报名费直接给开发团队
    ...
    returns(bool, uint256)
    {
        // make sure name fees paid 确定名字的费用
        require (msg.value >= registrationFee_, "umm.....  you have to pay the name fee");
        ...
        // if no affiliate code was given or player tried to use their own, lolz
        uint256 _affID;
        if (_affCode != "" && _affCode != _name) // 如果邀请人名字不为空，而且邀请人名字不等于自己的名字
        {
            // get affiliate ID from aff Code
            _affID = pIDxName_[_affCode];

            // if affID is not the same as previously stored // 如果邀请人的ID和之前最后一个邀请人的id不同，设置为新邀请人id
            if (_affID != plyr_[_pID].laff)
            {
                // update last affiliate
                plyr_[_pID].laff = _affID;
